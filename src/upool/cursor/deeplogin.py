...
from .models import KIND_SESSION, cookie_header_for, token_kind
...
def _open(
    ...
    """One seam the whole module goes through.
    ...
    opener - and whatever cookie jar it carries, or does not - is the caller's to
    build; this seam only threads it through, so the GET and the POST can share
    one jar while the poll gets none.
    """
...
def exchange(
    ...
    *,
    ...
    uuid = str(uuid4())
    cookie = cookie_header_for(user_id, web_token)

    # One jar for the GET and the POST: the GET's 307 seeds
    # cursor-web-target-synced-user, and the confirm needs it on the ride back.
    ...
    try:
        with _open(opener, jar_request, timeout):
            pass
    ...
    # they are the likely CSRF check. Same opener as the GET, so the jar cookie
    # the redirect just set rides along.
    ...
    try:
        with _open(opener, confirm, timeout) as response:
    ...
    # the confirm's redirect has registered. A different host, and no jar cookie
    # it needs, so it gets its own bare opener rather than the GET/POST one.
    poll_opener = urllib.request.build_opener()
    for attempt in range(attempts):
        minted = _poll_once(poll_opener, uuid, verifier, timeout)
...
def _poll_once(
    opener: urllib.request.OpenerDirector, uuid: str, verifier: str, timeout: float
) -> dict | None:
...
def _verify_identity(expected_user_id: str, payload: dict) -> DeepLogin:
    ...
    failure a switcher must never paper over. An empty ``authId`` is refused for
    the same reason: it is not proof of the right account, and treating "nothing
    to compare" as a pass would let the worst outcome through silently.
    """
    ...
    if not minted_user_id:
        raise UPoolError(
            "Cursor's sign-in poll did not identify an account - not switching."
        )
    if minted_user_id != expected_user_id:
        raise UPoolError(
